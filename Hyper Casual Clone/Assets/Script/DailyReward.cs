...
public class DailyReward : MonoBehaviour
{
    ...
    public Text remainingDailyText;
        ...
        if (initialized)
        {
            if (LevelController.Current.startMenu.activeInHierarchy)
            {
                ...
                else
                {
                    ...
                    remainingDailyText.text = string.Format("{0}:{1}:{2}", timeSpan.Hours.ToString("D2"), timeSpan.Minutes.ToString("D2"), timeSpan.Seconds.ToString("D2"));
